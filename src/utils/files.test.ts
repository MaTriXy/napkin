...
describe("listFiles", () => {
  test("lists all files", () => {
    const files = listFiles(vault.path);
  ...
  test("filters by extension", () => {
    const files = listFiles(vault.path, { ext: "md" });
  ...
  test("filters by folder", () => {
    const files = listFiles(vault.path, { folder: "Projects" });
  ...
  test("returns empty for nonexistent folder", () => {
    const files = listFiles(vault.path, { folder: "Nope" });
...
describe("listFolders", () => {
  test("lists folders", () => {
    const folders = listFolders(vault.path);
  ...
  test("filters by parent folder", () => {
    const folders = listFolders(vault.path, "Resources");
...
describe("resolveFile", () => {
  test("resolves by exact path", () => {
    const result = resolveFile(vault.path, "README.md");
    expect(result).toBe("README.md");
  });

  test("resolves by wikilink name", () => {
    const result = resolveFile(vault.path, "Active Projects");
  ...
  test("returns null for missing file", () => {
    const result = resolveFile(vault.path, "nonexistent-file");
...
describe("readFile", () => {
  test("reads file by wikilink name", () => {
    const { path, content } = readFile(vault.path, "README");
    expect(path).toBe("README.md");
    expect(content).toContain("My Vault");
  });

  test("reads file by exact path", () => {
    const { content } = readFile(vault.path, "Projects/Active Projects.md");
    expect(content).toContain("Project A");
  });

  test("throws for missing file", () => {
    expect(() => readFile(vault.path, "nonexistent")).toThrow("File not found");
  });
});

describe("getFileInfo", () => {
  test("returns file info", () => {
    const info = getFileInfo(vault.path, "README.md");
