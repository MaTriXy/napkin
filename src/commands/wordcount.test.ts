...
describe("wordcount", () => {
  test("counts words and characters", async () => {
    const data = await captureJson(() => wordcount({ json: true, vault: v.path, file: "note" }));
    expect(data.words).toBe(8);
    expect((data.characters as number)).toBeGreaterThan(0);
  });

  test("returns words only", async () => {
    const data = await captureJson(() => wordcount({ json: true, vault: v.path, file: "note", words: true }));
    expect(data.words).toBe(8);
    expect(data.characters).toBeUndefined();
  });

  test("returns characters only", async () => {
    const data = await captureJson(() => wordcount({ json: true, vault: v.path, file: "note", characters: true }));
