...
import { backlinks, links, unresolvedLinks, orphans, deadends } from "./links.js";
...
describe("backlinks", () => {
  test("finds backlinks to a file", async () => {
    const data = await captureJson(() => backlinks({ json: true, vault: v.path, file: "Alpha" }));
  ...
  test("returns total", async () => {
    const data = await captureJson(() => backlinks({ json: true, vault: v.path, file: "Alpha", total: true }));
...
describe("links", () => {
  test("lists outgoing links", async () => {
    const data = await captureJson(() => links({ json: true, vault: v.path, file: "index" }));
...
describe("unresolvedLinks", () => {
  test("finds unresolved links", async () => {
    const data = await captureJson(() => unresolvedLinks({ json: true, vault: v.path }));
...
describe("orphans", () => {
  test("finds files with no incoming links", async () => {
    const data = await captureJson(() => orphans({ json: true, vault: v.path }));
...
describe("deadends", () => {
  test("finds files with no outgoing links", async () => {
    const data = await captureJson(() => deadends({ json: true, vault: v.path }));
