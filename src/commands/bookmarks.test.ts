...
import { bookmarks, bookmark } from "./bookmarks.js";
...
describe("bookmarks", () => {
  test("lists bookmarks", async () => {
    const data = await captureJson(() => bookmarks({ json: true, vault: v.path }));
  ...
  test("returns total", async () => {
    const data = await captureJson(() => bookmarks({ json: true, vault: v.path, total: true }));
...
describe("bookmark", () => {
  test("adds a file bookmark", async () => {
    await captureJson(() => bookmark({ json: true, vault: v.path, file: "new.md", title: "New" }));
    const raw = fs.readFileSync(path.join(v.path, ".obsidian", "bookmarks.json"), "utf-8");
  ...
  test("adds a search bookmark", async () => {
    await captureJson(() => bookmark({ json: true, vault: v.path, search: "FIXME" }));
    const raw = fs.readFileSync(path.join(v.path, ".obsidian", "bookmarks.json"), "utf-8");
    const items = JSON.parse(raw);
  ...
  test("adds a URL bookmark", async () => {
    await captureJson(() => bookmark({ json: true, vault: v.path, url: "https://example.com" }));
    const raw = fs.readFileSync(path.join(v.path, ".obsidian", "bookmarks.json"), "utf-8");
    const items = JSON.parse(raw);
