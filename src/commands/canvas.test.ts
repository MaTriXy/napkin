...
import { canvases, canvasRead, canvasNodes, canvasCreate, canvasAddNode, canvasAddEdge, canvasRemoveNode } from "./canvas.js";
  ...
  v = createTempVault({
    "board.canvas": JSON.stringify({
      nodes: [
        ...
        { id: "eeff99001122", type: "group", x: -50, y: -50, width: 800, height: 400, label: "My Group" },
      ],
      edges: [
        { id: "edge00112233", fromNode: "aabb11223344", fromSide: "right", toNode: "ccdd55667788", toSide: "left", label: "links to" },
      ],
    }),
    "Notes/note.md": "# Note\nSome content",
  });
...
describe("canvases", () => {
  test("lists canvas files", async () => {
    const data = await captureJson(() => canvases({ json: true, vault: v.path }));
    expect(data.canvases).toContain("board.canvas");
  });

  test("returns total", async () => {
    const data = await captureJson(() => canvases({ json: true, vault: v.path, total: true }));
...
describe("canvasRead", () => {
  test("reads canvas with nodes and edges", async () => {
    const data = await captureJson(() => canvasRead({ json: true, vault: v.path, file: "board" }));
...
describe("canvasNodes", () => {
  test("lists all nodes", async () => {
    const data = await captureJson(() => canvasNodes({ json: true, vault: v.path, file: "board" }));
    expect((data.nodes as unknown[]).length).toBe(3);
  });

  test("filters by type", async () => {
    const data = await captureJson(() => canvasNodes({ json: true, vault: v.path, file: "board", type: "text" }));
...
describe("canvasCreate", () => {
  test("creates empty canvas", async () => {
    const data = await captureJson(() => canvasCreate({ json: true, vault: v.path, file: "new-board" }));
    expect(data.created).toBe(true);
    const content = JSON.parse(fs.readFileSync(path.join(v.path, "new-board.canvas"), "utf-8"));
...
describe("canvasAddNode", () => {
  test("adds a text node", async () => {
    const data = await captureJson(() => canvasAddNode({ json: true, vault: v.path, file: "board", type: "text", text: "New node" }));
    expect(data.added).toBe(true);
    expect(data.type).toBe("text");

    const content = JSON.parse(fs.readFileSync(path.join(v.path, "board.canvas"), "utf-8"));
  ...
  test("adds a file node", async () => {
    const data = await captureJson(() => canvasAddNode({ json: true, vault: v.path, file: "board", type: "file", noteFile: "Notes/note.md" }));
    expect(data.type).toBe("file");
  });

  test("adds a link node", async () => {
    const data = await captureJson(() => canvasAddNode({ json: true, vault: v.path, file: "board", type: "link", url: "https://example.com" }));
    expect(data.type).toBe("link");
  });

  test("auto-positions new nodes", async () => {
    await canvasAddNode({ json: true, vault: v.path, file: "board", type: "text", text: "Auto" });
    const content = JSON.parse(fs.readFileSync(path.join(v.path, "board.canvas"), "utf-8"));
...
describe("canvasAddEdge", () => {
  test("adds an edge between nodes", async () => {
    const data = await captureJson(() => canvasAddEdge({
      json: true, vault: v.path, file: "board",
      from: "aabb", to: "eeff", label: "belongs to",
    }));
    expect(data.added).toBe(true);
...
describe("canvasRemoveNode", () => {
  test("removes node and connected edges", async () => {
    const data = await captureJson(() => canvasRemoveNode({
      json: true, vault: v.path, file: "board", id: "aabb",
    }));
