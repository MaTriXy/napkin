 ...
 */
export function createTempVault(files?: Record<string, string>): { path: string; cleanup: () => void } {
  ...
  fs.writeFileSync(path.join(obsidianDir, "app.json"), JSON.stringify({ alwaysUpdateLinks: true }));
  fs.writeFileSync(
    path.join(obsidianDir, "daily-notes.json"),
    JSON.stringify({ folder: "Inbox/Daily", format: "YYYY-MM-DD", template: "Templates/Daily Note" }),
  );
